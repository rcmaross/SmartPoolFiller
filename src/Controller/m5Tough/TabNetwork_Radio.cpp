...
void onTimeSyncNotification(struct timeval *tv) {
    Serial.println("[NTP EVENT] Network time synchronization successfully completed!");

    // Extract the freshly synchronized network time structure
    time_t now = tv->tv_sec;
    struct tm* now_tm = localtime(&now);

    // Burn the verified time straight into the physical hardware clock cells
    M5.Rtc.setDateTime(now_tm);
    Serial.println("[RTC] Physical BM8563 cells hard-locked to verified network epoch.");
}
    ...
    if (sysState->connection_type == 1) {
        if (sysState->wifi_ssid.length() > 0) {
            ...
            if (sysState->ntp_sync_active) {
                ...
                configTzTime(sysState->tz_posix_rule.c_str(), "pool.ntp.org", "time.nist.gov");
            }
        }
        return;
    }
